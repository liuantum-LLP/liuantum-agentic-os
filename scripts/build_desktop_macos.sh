...
command -v xcode-select >/dev/null 2>&1 || missing+=("xcode-select")

echo "Liuant macOS desktop build helper"
echo "This script does not request privileged elevation and does not sign/notarize builds."
if ((${#missing[@]})); then
  printf 'Missing dependencies: %s\n' "${missing[*]}"
  echo "Install Xcode Command Line Tools with: xcode-select --install"
  echo "Install Rust from rustup.rs and Node.js 20+ before running a native build."
fi
